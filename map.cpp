...
void initMaps(void){
  // Compute the required size: SIZE_X * SIZE_Y * sizeOf(Type)
  const size_t CAPACITY = JSON_ARRAY_SIZE(250);

  // Allocate the memory for JSON document
  ...
  for (i = 0; i < arraySizeMap; i++){
    ...
    if(j !=  sizeX) Serial.println(F("ERROR MAP Size (X)"));    
  }
  if(i !=  sizeY) Serial.println(F("ERROR MAP Size (Y)"));
...
void printBuffersSerial(void){
  ...
  Serial.print(F("X size: "));
  Serial.println(sizeX);

  Serial.print(F("Y size: "));
...
void testInterpolateSimp(){
  ...
  verboseInterpolateSimp(x0, y0, x1, y2, x);
  Serial.println(F("Expected: 106.77"));
}
...
void testInterpolateFull() {
  ...
  Serial.println(F("interpolateFull(2421, 299) - OK: 268.75"));
  Serial.println(interpolateFull(2421, 299));

  Serial.println(F("interpolateFull(5196, 299) - OK: 561.89"));
  Serial.println(interpolateFull(5196, 299));

  Serial.println(F("interpolateFull(5196, 554) - OK: 12"));
  Serial.println(interpolateFull(5196, 554));

  Serial.println(F("interpolateFull(2250, 525) - OK: 19.25"));
  Serial.println(interpolateFull(2250, 525));

  Serial.println(F("interpolateFull(2250, 406) - OK: 25.83"));
  Serial.println(interpolateFull(2250, 406));

  Serial.println(F("interpolateFull(0, 299) - OK: 0"));
  Serial.println(interpolateFull(0, 299));

  Serial.println(F("interpolateFull(2250, 0) - OK: 212.5"));
